---
title: "Los usuarios como producto de los servicios digitales"
date: 2026-04-19T14:14:22-06:00
draft: false
tags: ["zettelkasten", "nota", "tecnologia", "capitalismo", "plataformas", "datos", "atencion", "redes-sociales", "cita"]
summary: "Igual que las mujeres entran gratis a las discotecas porque no son clientes sino parte del producto, los usuarios de servicios digitales no los consumen: son consumidos por ellos."
---

# Los usuarios como producto de los servicios digitales

Igual que las mujeres entran gratis a las discotecas porque no son clientes sino parte del producto, los usuarios de servicios digitales no los consumen: son consumidos por ellos.

## Idea
- La gratuidad de las plataformas no es generosidad: es el precio de tu atención y tus datos.
- La analogía discoteca/plataforma digital revela la lógica extractiva: el usuario es el recurso.
- Ver también: `zettel/doomscrolling-como-sintoma-estructural`

## Fuente
`lit/ernesto-castro`
