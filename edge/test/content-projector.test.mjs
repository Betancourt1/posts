...
test("projects the complete repository with the migration count contract", async () => {
  ...
  assert.equal(documents.length, 310);
  assert.equal(documents.filter((document) => document.searchable).length, 289);
  assert.equal(routes.filter((route) => route.kind === "canonical").length, 310);
