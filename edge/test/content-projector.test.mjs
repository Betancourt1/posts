...
test("projects the complete repository with the migration count contract", async () => {
  ...
  assert.equal(files.length, 377);
  assert.equal(documents.length, 477);
  assert.equal(documents.filter((document) => document.searchable).length, 444);
  assert.equal(routes.filter((route) => route.kind === "canonical").length, 477);
