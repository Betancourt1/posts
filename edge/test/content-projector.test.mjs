...
test("projects the complete repository with the migration count contract", async () => {
  ...
  assert.equal(files.length, 275);
  assert.equal(documents.length, 371);
  assert.equal(documents.filter((document) => document.searchable).length, 349);
  assert.equal(routes.filter((route) => route.kind === "canonical").length, 371);
