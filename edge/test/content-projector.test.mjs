...
test("projects the complete repository with the migration count contract", async () => {
  ...
  assert.equal(documents.length, 385);
  assert.equal(documents.filter((document) => document.searchable).length, 363);
  assert.equal(routes.filter((route) => route.kind === "canonical").length, 385);
  assert.equal(routes.filter((route) => route.kind === "alias").length, 93);
});
