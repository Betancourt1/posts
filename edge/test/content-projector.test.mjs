...
test("projects the complete repository with the migration count contract", async () => {
  ...
  assert.equal(files.length, 375);
  assert.equal(documents.length, 473);
  assert.equal(documents.filter((document) => document.searchable).length, 440);
  assert.equal(routes.filter((route) => route.kind === "canonical").length, 473);
