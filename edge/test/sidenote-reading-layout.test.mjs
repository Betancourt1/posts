...
test("reading rail places the graph above notes without a header trigger", async () => {
  ...
  assert.doesNotMatch(client, /useRail[\s\S]*?classList\.contains\("zen-mode"\)/);
  ...
  assert.match(client, /new ResizeObserver\(schedulePosition\)\.observe\(graph\)/);
  assert.match(client, /graph\.addEventListener\("transitionend"[\s\S]*?event\.propertyName === "max-height"[\s\S]*?schedulePosition\(\)/);
  ...
  assert.match(css, /\.sidenote-sidebar-graph\s*\{[\s\S]*?transition-property:\s*opacity, max-height, margin-bottom, transform, visibility;[\s\S]*?transition-duration:\s*0\.28s, 0\.42s, 0\.42s, 0\.42s, 0s;/);
  ...
  assert.match(css, /body\.has-sidenotes\.zen-mode \.sidenote-sidebar-graph,[\s\S]*?body\.has-sidenotes\.zen-mode-returning \.sidenote-sidebar-graph\s*\{[\s\S]*?max-height:\s*0;[\s\S]*?opacity:\s*0;[\s\S]*?transition-delay:\s*0s, 0s, 0s, 0s, 0\.42s;/);
  assert.match(css, /@media \(prefers-reduced-motion: reduce\)[\s\S]*?transition-duration:\s*0\.12s, 0s, 0s, 0s, 0s;[\s\S]*?transition-delay:\s*0s, 0\.12s, 0\.12s, 0s, 0\.12s;/);
