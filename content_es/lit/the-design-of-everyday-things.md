---
title: "Don Norman — The Design of Everyday Things"
date: 2026-07-15T00:05:33-06:00
draft: false
tags: ["quote","design","usability","technology"]
summary: "Two of the most important characteristics of good design are discoverability and understanding."
quotes:
  - text: "A door would seem to be about as simple a device as possible."
    author: "Don Norman"
    source: "The Design of Everyday Things"
  - text: "Two of the most important characteristics of good design are discoverability and understanding."
    author: "Don Norman"
    source: "The Design of Everyday Things"
  - text: "[...] the designer must provide signals that naturally indicate where to push. These need not destroy the aesthetics. Put a vertical plate on the side to be pushed. Or make the supporting pillars visible. The vertical plate and supporting pillars are natural signals, naturally interpreted, making it easy to know just what to do: no labels needed."
    author: "Don Norman"
    source: "The Design of Everyday Things"
  - text: "Design is concerned with how things work, how they are controlled, and the nature of the interaction between people and technology."
    author: "Don Norman"
    source: "The Design of Everyday Things"
  - text: "When people fail to follow these bizarre, secret rules, and the machine does the wrong thing, its operators are blamed for not understanding the machine, for not following its rigid specifications."
    author: "Don Norman"
    source: "The Design of Everyday Things"
  - text: "We were designing things for people, so we needed to understand both technology and people."
    author: "Don Norman"
    source: "The Design of Everyday Things"
  - text: "Feedback must be inmediate: even a delay of a tenth of a second can be disconcerting. If the day is too long, people often give up, going off to do other activities. This is annoying to the people, but it can also be wasteful of resources when the system spends considerable time and effort to satisfy the request, only to find that the intended recipient is no longer there."
    author: "Don Norman"
    source: "The Design of Everyday Things"
  - text: "Poor feedback can be worse than no feedback at all, because it is distracting, uninformative, and in many cases irritating and anxiety-provoking."
    author: "Don Norman"
    source: "The Design of Everyday Things"
  - text: "A good conceptual model allows to predict the effects of our action."
    author: "Don Norman"
    source: "The Design of Everyday Things"
  - text: "The same technology that simplifies life by providing more functions in each device also complicates life by making the device harder to learn, harder to use. This is the paradox of technology and the challenge for the designer."
    author: "Don Norman"
    source: "The Design of Everyday Things"
---

# Don Norman — *The Design of Everyday Things*

> A door would seem to be about as simple a device as possible.

> Two of the most important characteristics of good design are *discoverability* and *understanding*.

> [...] the designer must provide signals that naturally indicate where to push. These need not destroy the aesthetics. Put a vertical plate on the side to be pushed. Or make the supporting pillars visible. The vertical plate and supporting pillars are natural signals, naturally interpreted, making it easy to know just what to do: no labels needed.

> Design is concerned with how things work, how they are controlled, and the nature of the interaction between people and technology.

> When people fail to follow these bizarre, secret rules, and the machine does the wrong thing, its operators are blamed for not understanding the machine, for not following its rigid specifications.

> We were designing things for people, so we needed to understand both technology and people.

> Feedback must be inmediate: even a delay of a tenth of a second can be disconcerting. If the day is too long, people often give up, going off to do other activities. This is annoying to the people, but it can also be wasteful of resources when the system spends considerable time and effort to satisfy the request, only to find that the intended recipient is no longer there.

> Poor feedback can be worse than no feedback at all, because it is distracting, uninformative, and in many cases irritating and anxiety-provoking.

> A good conceptual model allows to predict the effects of our action.

> The same technology that simplifies life by providing more functions in each device also complicates life by making the device harder to learn, harder to use. This is the paradox of technology and the challenge for the designer.

